...
class Env2D:
    ...
    def initialize(self, envfile, params):
        ...
        try:
            ...
            print(
                "File doesn't exist. Please use correct naming convention for database eg. 0.png, 1.png .. and so on. You gave, %s"
                % (envfile)
            )
        self.x_lims = params["x_lims"]
        self.y_lims = params["y_lims"]

        self.x_res = (self.x_lims[1] - self.x_lims[0]) / (
            (self.image.shape[1] - 1) * 1.0
        )
        self.y_res = (self.y_lims[1] - self.y_lims[0]) / (
            (self.image.shape[0] - 1) * 1.0
        )

        orig_pix_x = math.floor(
            0 - self.x_lims[0] / self.x_res
        )  # x coordinate of origin in pixel space
        orig_pix_y = math.floor(
            0 - self.y_lims[0] / self.y_res
    ...
    def in_limits(self, state):
        ...
        pix_x, pix_y = self.to_image_coordinates(state)
        if (
            0 <= pix_x < self.image.shape[1]
            and 0 <= pix_y < self.image.shape[0]
            and self.x_lims[0] <= state[0] < self.x_lims[1]
            and self.y_lims[0] <= state[1] < self.y_lims[1]
        ):
            return True
        return False
    ...
    def to_image_coordinates(self, state):
        ...
        pix_y = int(
            self.image.shape[1]
            - 1
            - (self.orig_pix[1] + math.floor(state[1] / self.y_res))
        )
        return (pix_x, pix_y)
    ...
    def calculate_distance_transform(self, sampling=[1, 1]):
        if not self.distance_transform_available:
            sampling_pix = np.divide(
                np.array(sampling), np.array([self.x_res, self.y_res])
            )
    ...
    def visualize_environment(self):
        ...
        self.axes.imshow(
            self.image,
            extent=(self.x_lims[0], self.x_lims[1], self.y_lims[0], self.x_lims[1]),
            cmap="gnuplot",
        )
