...
class GreedyPlanner(SearchBasedPlanner):
    ...
    def plan(self, max_expansions=100000):
        ...
        ), "Planner has not been initialized properly. Please call initialize or reset_problem function before plan function"
        start_t = time.time()
        self.came_from[self.start_node] = (None, None)
        self.cost_so_far[self.start_node] = 0.0
        ...
        while not self.frontier.empty():
            ...
            neighbors, edge_costs, valid_edges, invalid_edges = self.get_successors(
                curr_node
            )
            ...
            for i, neighbor in enumerate(neighbors):
                ...
                if neighbor not in self.visited:
                    if (
                        neighbor not in self.cost_so_far
                        or new_g <= self.cost_so_far[neighbor]
                    ):
        ...
        if found_goal:
            path, path_cost = self.reconstruct_path(
                self.came_from, self.start_node, self.goal_node, self.cost_so_far
            )
        else:
