...
class SearchBasedPlanner(ABC):
    ...
    def initialize(self, problem: PlanningProblem):
        """Initialize the planner with some planning problem"""
        assert (
            problem.initialized == True
        ), "Planning problem data structure has not been initialized"
        ...
        if self.visualize:
            self.env.initialize_plot(
                self.lattice.node_to_state(self.start_node),
                self.lattice.node_to_state(self.goal_node),
            )  # , grid_res = [self.lattice.resolution[0], self.lattice.resolution[1]])
    ...
    def get_successors(self, node):
        ...
        invalid_edges = (
            []
        )  # Continuous states that are first states in collision along an edge
    ...
    def get_predecessors(self, node):
        ...
        for i, pred in enumerate(preds):
            pred_node = pred[0]
            pred_edge = pred[1]
            isvalid, first_coll_state = self.env.is_edge_valid(
                pred_edge
            )  # first_coll_state will be empty if the state simply lies outside workspace limits
            if not isvalid:
                if first_coll_state:
