...
class XYAnalyticLattice(StateLattice):
    def __init__(self, params):
        ...
        self.rotation = params[
            "rotation"
        ]  # rotation of lattice about z-axis w.r.t world
    ...
    def state_to_node(self, state):
        ...
        node = np.array(
            [
                cos(self.rotation) * node[0] + sin(self.rotation) * node[1],
    ...
    def get_edge(self, parent_node, child_node):
        ...
        num_points = (
            self.distance_bw_states(parent_state, child_state) / self.path_resolution
        )
    ...
    def get_successors(self, node):
        ...
        for it in self.children:
            ...
            child_state = self.node_to_state(child_node)
            edge = self.interpolate(
                parent_state,
                child_state,
                self.distance_bw_states(parent_state, child_state)
    ...
    def get_predecessors(self, node):
        ...
        for it in self.predecessors:
            ...
            edge = self.interpolate(
                child_state,
                parent_state,
                self.distance_bw_states(child_state, parent_state)
                / self.path_resolution,
            )
    ...
    def interpolate(self, s1, s2, num_points):
        ...
        while True:
            ...
            temp = [
                s1[i] + (s2[i] - s1[i]) * (curr_len / length) for i in range(self.ndims)
            ]
            # Update curr_len
            curr_len += d_l
            if curr_len > length:
