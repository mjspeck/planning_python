...
from math import copysign, cos, floor, pi, radians, sin, sqrt
...
if TYPE_CHECKING:
    from planning_python.utils.types import Node, State
...
class XYHAnalyticLattice(StateLattice):
    def __init__(self, params):
        ...
        self.connectivity = params[
            "connectivity"
        ]  # Connectivity of the lattice i.e dubins_turn_90 or dubins_turn_45 etc.
        ...
        self.path_resolution = params[
            "path_resolution"
        ]  # Resolution to do collision checking at

        if self.connectivity == "dubins_turn_90":
            self.num_heading = 4
            self.resolution = np.array(
                [params["radius"], params["radius"], np.pi / 2.0]
            )  # Resolution of lattice in different directions
            ...
            raise NotImplementedError
            # define children and predecessors for reeds schepp 90 degree turns
        elif self.connectivity == "reeds_scehpp_turn_45":
            raise NotImplementedError
            # least priority right now

        StateLattice.__init__(
            self,
            self.ndims,
            [self.x_lims[0], self.y_lims[0], 0],
            [self.x_lims[1], self.y_lims[1], 2.0 * np.pi],
            self.resolution,
        )
    ...
    def state_to_node(self, state: State) -> Node:
        ...
        pos = np.array(
            [
                cos(self.rotation) * pos[0] + sin(self.rotation) * pos[1],
        ...
        rot = int(
            (
                (angles.normalize_angle_positive(state[2]) / 2.0 * np.pi)
                * self.num_heading
            )
            % self.num_heading
        )
    ...
    def enumerate_lattice(self):
        ...
        assert (
            len(node_to_succs.keys()) == self.total_cells
        ), "Did not enumerate all possible successor cells"
        assert (
            len(node_to_preds.keys()) == self.total_cells
        ), "Did not enumerate all possible predecessor cells"
